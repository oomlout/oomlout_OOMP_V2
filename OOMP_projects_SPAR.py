...
def farmProjects():
    print("Farming Projects for :" + "SPAR")
    genFile = "OOMP_projects_SPAR_gen.py"
    repos = OOMP_projects_BASE.getRepos(user="sparkfun")
    print("    Found " + str(len(repos)) + " repos")
    oomMakeDir("json/")
    filename = "json/oomp_Projects_SPAR_repoRaw.json"
    oomWriteToFile(filename,json.dumps(repos))

    

def makeBaseProjects():
    print("Harvesting Sparkfun Projects")
    repoString = ""
    filename = "json/oomp_Projects_SPAR_repoRaw.json"
    repos = json.loads(oomReadFileToString(filename))
    projects = []
    exportFile = "OOMP_projects_SPAR_generated.py"
    contents = ""
    contents = contents + """
    "###### THIS FILE IS GENERATED BY OOMP_projects_SPAR.py"
    OOMP_projects_BASE
    ...
    for repo in repos:
        ...
        outDir = oomGitPullNew(repo,"sourceFiles/projects/")  
        ...
        d = base.copy()
        d["repo"] = repo.replace("https://github.com/sparkfun/","")
        processDirSparkfun(outDir,d)
